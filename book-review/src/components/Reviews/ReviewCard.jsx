...
function ReviewCard({returnedReview, user, handleDeleteClick}) {
  ...
  return ( 
    <div className='review bg-purple-500 text-white rounded-lg'>
      {returnedReview.comment}
      { 
        user.id == returnedReview.user_id ? 
        (
          <div>
            <ArchiveBoxXMarkIcon onClick={(e) => handleDeleteClick(returnedReview.id)} className='h-9 w-9 icon'/>
      ...
      <div className="relative w-auto my-6 mx-auto max-w-3xl">
        ...
        <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
          ...
          <form className='max-w-[400px] w-full mx-auto rounded-lg bg-purple-500 p-8 px-8'>
                <input type="text" className='w-full p-2 text-purple rounded-lg bg-white mt-2 focus:border-blue-500 focus:bg-white focus:outline-none' value={updatedReview} id="comment" name="name" onChange={e => setUpdatedReview(e.target.value)} required placeholder='Leave your review' />
                <button type="submit" className='w-full my-5 py-2 bg-white border-purple text-purple font-semibold rounded-lg' onClick={(e) => handleUpdateClick(e)}>POST</button>
            </form>
          </div>
          {/*footer*/}
